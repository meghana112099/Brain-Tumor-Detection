...
class Gui:
    ...
    def __init__(self):
        ...
        RB3 = tkinter.Radiobutton(self.FirstFrame.getFrames(), text="Display Accuracy",
                                  variable=self.val, value=3, command=self.check)
        RB3.place(x=250, y=300)

        browseBtn = tkinter.Button(self.FirstFrame.getFrames(), text="Browse", width=8, command=self.browseWindow)
        browseBtn.place(x=500, y=520)
    ...
    def check(self):
        ...
        if (self.val.get() == 1):
            ...
            if res > 0.5:
                ...
                resLabel1 = tkinter.Label(self.FirstFrame.getFrames(), text="Classification: YES", height=1, width=20)

                resLabel.configure(background="White", font=("Comic Sans MS", 13, "bold"), fg="red")
                resLabel1.configure(background="White", font=("Comic Sans MS", 13, "bold"), fg="red")

            else:
                resLabel = tkinter.Label(self.FirstFrame.getFrames(), text="Tumor not Detected", height=1, width=20)
                resLabel.configure(background="White", font=("Comic Sans MS", 13, "bold"), fg="green")
                resLabel1 = tkinter.Label(self.FirstFrame.getFrames(), text="Classification: NO", height=1, width=20)
                resLabel1.configure(background="White", font=("Comic Sans MS", 13, "bold"), fg="green")

            resLabel.place(x=480, y=380)
            resLabel1.place(x=480, y=430)
            ...
            res = predictTumor(mriImage)
            if res>0.5:
              print(cnn.fit(x=training_set, validation_data=test_set, epochs=5))
